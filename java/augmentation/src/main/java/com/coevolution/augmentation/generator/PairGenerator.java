...
public class PairGenerator {
    ...
    public void generate() throws Exception {
        ...
        System.out.println("[PairGenerator] " + v1Files.size() + " fichiers v1.ecore trouves");
        System.out.println("[PairGenerator] Max mutations/fichier : " + maxMutations);

        for (File v1 : v1Files) {
            try { generatePairsForFile(v1); }
            catch (Exception e) {
                System.err.println("[ERREUR] " + v1.getName() + " : " + e.getMessage());
                totalFailed++;
            }
        }
    ...
    private void generatePairsForFile(File v1File) throws Exception {
        EPackage original = engine.loadEcore(v1File);
        List<EcoreMutationEngine.MutationCandidate> candidates =
                engine.generateMutations(original);
        int count = 0;
        for (EcoreMutationEngine.MutationCandidate c : candidates) {
            ...
            pairDir.mkdirs();
            engine.saveEcore(engine.clonePackage(original), new File(pairDir, "v1.ecore"));
